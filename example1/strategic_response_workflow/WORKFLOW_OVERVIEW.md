...
| 层 | 管什么 | 载体 |
|---|---|---|
| **引擎** runner | 顺序、IO、评分门循环、暂停/续跑、审稿者选择、导出、归档 | `workflow/runner.py` |
| **体例模块** | 分析路径：检索口径、各步提示词、文章模板、示例稿、检索类型 | `report_modules/<体例>/` |
| **reasoning_dna** | 想到哪几层（判断纪律，"刀"）+ 共享词汇（证据分级/词表/强度门） | 共享元格式 `reasoning_dna/conventions.md` + `shared_schema/`（evidence_maturity/register_blacklist/strength_gate，2026-07-05 新建）+ 各体例自带 `report_modules/<体例>/reasoning_dna/*.md`（gated 注入判断步） |
...
| **audience_profile** | 读者层级站位（写给谁看：判断高度与对策粒度） | `audience_profiles/{national_leader,moe_leadership,bureau}.md`（`--audience-level` 显式选择，gated，注入同 institution 五步；2026-07-05 新建） |
| **质量门** | 什么算合格（六维评分 + 硬伤集 + 分档） | `quality_rubric.md`（唯一评分真源）+ runner `_parse_review_verdict` 评分门 + `reviewers/jiaokeyuan_review_scope.md`（终审人格） |

**横向**是数据流（事实→判断→构思→文字→审核），**纵向**是各层各管一段、各自可演化、互不绑死。
其中 reasoning_dna（含 shared_schema）/ policy_style_dna（含 quality_rubric）/ institution_profile / audience_profile 都是**运行时加载、gated（无文件/未选择则零注入）、每次运行快照留痕**。
...
### 阶段 0 · 路由与初始化　〔引擎〕
- **init**：按 `--report-type` 载入体例模块（`report_modules/<type>/module.yaml`），建运行目录，**快照**本次要用的全部规则到 `runs/<id>/`——`policy_style_dna_snapshot/`（wiki 规则 + `quality_rubric.md` 评分标尺）、`reasoning_dna_snapshot/`（刀 + conventions + shared_schema 三件）、`institution_profile_snapshot.md`、`audience_profile_snapshot.md`（选了 `--audience-level` 时），写 `input.yaml`（含 audience_level）。
...
### 阶段 2 · 判断（把事实抬成判断）　〔体例模块 + reasoning_dna + shared_schema〕
只输出**可检查的中间产物**，不写正文、不吐思维链。判断步注入的刀前置拼接 shared_schema（统一证据成熟度 E0–E4、军事化词表、强度双钥门），全体刀同一套字号。
- **redefine_task** → `judgment_outputs/task_redefinition.md`：把”材料题”重定义成”判断题”（注入 institution_profile + audience profile〔选了读者层级时〕，按机构站位与读者层级把真问题框到位）。
...
### 阶段 3 · 构思（判断→结构与建议）　〔体例模块 + reasoning_dna + institution_profile + audience_profile〕
...
### 阶段 4 · 写作　〔体例模块 + policy_style_dna + institution_profile + audience_profile〕
- **write_draft** → `drafts/current_article.md`：模块 `prompts/writing.md` + `templates/article_template.md` + **注入 policy_style_dna(writing)** + **institution_profile（对策锁定落点域）** + **audience profile（判断高度与对策粒度对档，选了读者层级时）** + 全部判断/构思产物 + 检索材料。
...
- **审稿契约（reviewer 无关）**：暂停时落盘 `review_io/request_iter{n}.json`（机读）+ `INSTRUCTION_iter{n}.md`（自然语言交接）+ 输出格式契约——**推荐报告末尾附 rubric 评分 JSON 块（`quality_rubric.md` 第五节，评分门优先解析）**；无 JSON 时回退 `总体结论：达到|基本达到|未达到` 行匹配，老报告不崩。换审稿者只换触发方式，契约不变。
...
- **reasoning_dna 注入（gated）**：各体例 `module.yaml` 的 `reasoning_dna_injection` 声明 step→刀映射，runner `_with_reasoning_dna` 把 **shared_schema 三件（前置）+ 共享 conventions + 刀** 拼到对应判断步；无刀则零注入。当前三体例齐：strategic(intent/threat/hedge)、experience(cause/efficacy)、trend(signal/driver/stage/uncertainty)。
...
- **audience_profile 注入（gated、显式选择）**：`--audience-level` 选定后，`_with_audience_profile` 把对应档（关注问题/宏中微权重/语气详略/反例句对）紧随 institution_profile 注入同五步；不选则字节级零变化。站位质量由 `audience_alignment` scope 与终审维度 3 校验（读者层级 ≠ 政策对象，两概念隔离）。
...
| 改读者站位档 | 改 `audience_profiles/<level>.md`（或新增档位 + argparse choices 加一项） |
| 改文风标准 | 改 `policy_style_dna/wiki/*`、`policy_style_dna/review_scope.md`（均在仓根） |
| 改"什么算合格" | 改 `quality_rubric.md`（评分门/终审/打分工具/盲测同源生效）；改完对 `method_audit/samples/` 锚点复跑打分防回归 |
| 改证据分级 / 词表 / 强度门 | 改 `shared_schema/{evidence_maturity,register_blacklist,strength_gate}.md`（全体刀同步生效，勿在刀内重定义） |
...
- 推理层共享：`reasoning_dna/conventions.md`、`reasoning_dna/review_scope.md`、`shared_schema/{evidence_maturity,register_blacklist,strength_gate,layer_ownership}.md`
- 文风层（仓根）：`../policy_style_dna/wiki/`、`../policy_style_dna/review_scope.md`
- 机构层（仓根）：`../institution_profile.md`、`institution_profile/CORPUS_RECOMMENDATION_LOGIC.md`（对策生成思路，从38篇蒸馏）
- 读者站位层：`audience_profiles/{national_leader,moe_leadership,bureau}.md`
- 质量门：`quality_rubric.md`（唯一评分真源）+ `review_gate_result.json`（每 run 留痕）
- 审稿：`reviewers/instruction_template.md`；五个 scope = style_and_expression / reasoning_compliance / precedent_check / **jiaokeyuan_final_review**（教科院终审，`reviewers/jiaokeyuan_review_scope.md`）/ **audience_alignment**（站位互校验，`reviewers/audience_alignment_scope.md`）
- 配套 skill：`.claude/skills/{distill-reasoning-dna, audit-extend-suggestions}/`、仓根 `../report-clarify/`（写作前澄清）
- 独立工具：`tools/score_article.py`（单篇终审打分）、`tools/suggestion_audit_extend.py`（建议核查—延展硬判定）
- 运行产物：`runs/<run-id>/`（`judgment_outputs/`、`drafts/`、`review_reports/`、`review_io/`、`suggestion_audit/`、`*_snapshot*`、`review_gate_result.json`、`run_state.json`、`run_log.md`）
...
| 名称 | 类型 | 干什么 | 位置 | 进主链路？ |
|---|---|---|---|---|
| **report-clarify** | skill | 写作前澄清（S1）：预检索→候选研究问题→五问点选（读者层级/体例/观点/维度/风格）→拼完整启动命令，用户拍板 | 仓根 `../report-clarify/` | 否（启动前用，产出即启动命令） |
