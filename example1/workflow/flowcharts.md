...
flowchart TD
  ...
  I --> J["组装报告骨架<br/>功能：编排"]
  J --> K["最终证据审查<br/>功能：审查"]
  K --> L["全文内容结构审查<br/>功能：审查"]
  L --> M["根据审查意见修改全文<br/>功能：写作/修改"]
  M --> N["归档 run_log.md<br/>功能：运行日志"]
```
...
全文审核 agent 读取完整报告和固定审查模板，先判断内容结构是否合理，再据此修改全文。

```mermaid
flowchart TD
  A["final_report.md<br/>功能：待审查全文"] --> C["全文内容结构审查<br/>执行：DeepSeek v4 Pro<br/>功能：审查"]
  B["content_structure_review.md<br/>功能：固定审查模板"] --> C
  C --> D["fulltext_content_structure_review.md<br/>功能：审查报告"]
  A --> E["全文内容结构修改<br/>执行：DeepSeek v4 Pro<br/>功能：写作/修改"]
  B --> E
  D --> E
  E --> F["final_report_reviewed.md<br/>功能：修改后全文"]
```

审查边界：

- 只处理文章内容、结构、比较逻辑、论证链条、政策建议和政策文体。
- 不处理引用编号、参考文献格式、脚注尾注格式和文件来源格式。
- 不新增未经原文或 NotebookLM 检索材料支撑的国外政策事实。
