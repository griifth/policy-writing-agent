...
def _prompt_text(topic: str, query_type: str, expected_fields: list[str]) -> str:
    fields = "\n".join(f'  "{field}": ""' for field in expected_fields)
    return (
        f"请基于当前 NotebookLM 知识库，围绕“{topic}”执行 {query_type} 分析。\n"
        ...
        jobs.append(
            {
                ...
                "prompt_file": str(Path("prompts") / prompt_file),
                "prompt": _prompt_text(topic, query_type, expected_fields),
