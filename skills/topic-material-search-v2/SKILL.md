...
- **用户供料入参（2026-07-20 增，两模式通用）**：本单存在 `runs/<id>/outputs/供料编目.md` 时——侦察模式先读 U 料再联网（态势速览「你给的料」栏回显、供料佐证的候选问题/判型证据标 U 锚）；全量模式**检索计划先读供料编目、按主线段先铺 U 料、只对缺口段联网补搜**（U 料内容与关键词可正常进入检索查询词，用于查证/溯源/扩展）；同一事实 U 料与网搜矛盾时两方并陈交料包、标注分歧。
...
- **无联网权限**：网搜部分标 `[SIMULATED]`（用户供料为真实材料不打标），据模型常识构造合理态势供流程测试，交付时显著告知"网搜侦察为模拟，真实检索待补测"。

## Scout 流程（一趟，分钟级）

1. **接主题**：入参仅需 `topic`（可选 `notes` 传用户已知偏好；本单有 `outputs/供料编目.md` 时先读它、U 料先入速览）。不做 co-design 问答（那是全量模式的事）。
